function stCodesTable = makeStTable(settings)
...
//--------------------------------------------------------------------------
  ...
  stCodesTable = zeros(1, samplesPerCode);
  
  //--- Find time constants ------------------------------------------------
  ts = 1/settings.samplingFreq;   // Sampling period in sec
  tc = 1/settings.codeFreqBasis;  // GLONASS ST-code chip period in sec
  
  //--- Generate GLONASS ST code -----------------------------------------
  stCode = generateSTcode();
  
  //=== Digitizing =======================================================
  
  //--- Make index array to read ST code values -------------------------
  // The length of the index array depends on the sampling frequency -
  // number of samples per millisecond (because one ST code period is one
  // millisecond).
  codeValueIndex = ceil((ts * (1:samplesPerCode)) / tc);
  
  //--- Correct the last index (due to number rounding issues) -----------
  codeValueIndex($) = settings.codeLength;
  //--- Make the digitized version of the ST code -----------------------
  // The "upsampled" code is made by selecting values form the ST code
  // chip array (stCode) for the time instances of each sample.
  stCodesTable(1, :) = stCode(codeValueIndex);
