...
//--------------------------------------------------------------------------
  ...
  bar(acqResults.peakMetric, 'blue');

  xtitle('Acquisition results', 'PRN number (no bar - SV is not in the acquisition list)', 'Acquisition Metric');

  // Mark acquired signals ==================================================

  acquiredSignals = acqResults.peakMetric .* (acqResults.carrFreq ~= 0);

  bar (acquiredSignals, 'green');
