...
// Initialize result structure ============================================
  ...
  trackResults.I_P            = zeros(1, settings.msToProcess/4);
  trackResults.I_E            = zeros(1, settings.msToProcess/4);
  trackResults.I_L            = zeros(1, settings.msToProcess/4);
  
  // Outputs from the correlators (Quadrature-phase):
  trackResults.Q_E            = zeros(1, settings.msToProcess/4);
  trackResults.Q_P            = zeros(1, settings.msToProcess/4);
  trackResults.Q_L            = zeros(1, settings.msToProcess/4);
  ...
  for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
      ...
      loopCnt_dllDiscr       = ones(1,  settings.msToProcess/4);
      loopCnt_dllDiscrFilt   = ones(1,  settings.msToProcess/4);
      loopCnt_pllDiscr       = ones(1,  settings.msToProcess/4);
      loopCnt_pllDiscrFilt   = ones(1,  settings.msToProcess/4);
      loopCnt_I_E            = zeros(1, settings.msToProcess/4);
      loopCnt_I_P            = zeros(1, settings.msToProcess/4);
      loopCnt_I_L            = zeros(1, settings.msToProcess/4);
      loopCnt_Q_E            = zeros(1, settings.msToProcess/4);
      loopCnt_Q_P            = zeros(1, settings.msToProcess/4);
      loopCnt_Q_L            = zeros(1, settings.msToProcess/4);
...
// Read next block of data ------------------------------------------------            
        ...
        blksize = ceil((loopCnt_codeLength-remCodePhase) / codePhaseStep);//~64000
...
// Set up all the code phase tracking information -------------------------
        ...
        remCodePhase = (tcode(blksize) + codePhaseStep) - 8184.0;
        if (loopCnt==100) then
          //pause;
        end
