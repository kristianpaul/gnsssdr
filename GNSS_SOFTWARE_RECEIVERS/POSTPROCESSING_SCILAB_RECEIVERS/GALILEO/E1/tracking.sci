...
// Initialize result structure ============================================
  ...
  trackResults.meandrFreq     = ones(1, settings.msToProcess).*%inf;
  
  // Frequency of the tracked carrier wave:
  trackResults.carrFreq       = ones(1, settings.msToProcess).*%inf;
  
  // Outputs from the correlators (In-phase):
  trackResults.I_E_P          = zeros(1, settings.msToProcess);
  trackResults.I_P_E          = zeros(1, settings.msToProcess);
  trackResults.I_P_P          = zeros(1, settings.msToProcess);
  trackResults.I_P_L          = zeros(1, settings.msToProcess);
  trackResults.I_L_P          = zeros(1, settings.msToProcess);
    
  // Outputs from the correlators (Quadrature-phase):
  trackResults.Q_E_P          = zeros(1, settings.msToProcess);
  trackResults.Q_P_E          = zeros(1, settings.msToProcess);
  trackResults.Q_P_P          = zeros(1, settings.msToProcess);
  trackResults.Q_P_L          = zeros(1, settings.msToProcess);
  trackResults.Q_L_P          = zeros(1, settings.msToProcess);
  
  // Loop discriminators
  trackResults.dllDiscr       = ones(1, settings.msToProcess).*%inf;
  trackResults.dllDiscrFilt   = ones(1, settings.msToProcess).*%inf;
  trackResults.sllDiscr       = ones(1, settings.msToProcess).*%inf;
  trackResults.sllDiscrFilt   = ones(1, settings.msToProcess).*%inf;
  ...
  for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
      ...
      E1BCodeSplit(1,:) = [E1BCode($)    E1BCode(1:1023)    E1BCode(1024)];
      E1BCodeSplit(2,:) = [E1BCode(1023) E1BCode(1024:2046) E1BCode(2047)];
      E1BCodeSplit(3,:) = [E1BCode(2046) E1BCode(2047:3069) E1BCode(3070)];
      E1BCodeSplit(4,:) = [E1BCode(3069) E1BCode(3070:4092) E1BCode(1)];
      
      //Make the same for the meandr signal:
      // Get a vector with the meandr code sampled 1x/chip
      meandr = ones(1, settings.meandrLength/4); meandr(2:2:$) = -1;
      // Then make it possible to do early and late versions
      ///meandr = [meandr($) meandr meandr(1)];
      //Potentially buggy place. Advanced research required...
      meandr = [meandr($-1) meandr($) meandr meandr(1) meandr(2)];
      ...
      remCodePhase  = 0.0;
      // Current segment of the split into 4 parts memory code:
      currE1BCodeSegment  = 1;
      ...
      loopCnt_meandrFreq     = ones(1,  settings.msToProcess);
      loopCnt_absoluteSample = zeros(1, settings.msToProcess);
      loopCnt_dllDiscr       = zeros(1, settings.msToProcess);
      loopCnt_dllDiscrFilt   = zeros(1, settings.msToProcess);
      loopCnt_sllDiscr       = zeros(1, settings.msToProcess);
      loopCnt_sllDiscrFilt   = zeros(1, settings.msToProcess);
      loopCnt_pllDiscr       = zeros(1, settings.msToProcess);
      loopCnt_pllDiscrFilt   = zeros(1, settings.msToProcess);
      loopCnt_I_E_P          = zeros(1, settings.msToProcess);
      loopCnt_I_P_E          = zeros(1, settings.msToProcess);
      loopCnt_I_P_P          = zeros(1, settings.msToProcess);
      loopCnt_I_P_L          = zeros(1, settings.msToProcess);
      loopCnt_I_L_P          = zeros(1, settings.msToProcess);
      loopCnt_Q_E_P          = zeros(1, settings.msToProcess);
      loopCnt_Q_P_E          = zeros(1, settings.msToProcess);
      loopCnt_Q_P_P          = zeros(1, settings.msToProcess);
      loopCnt_Q_P_L          = zeros(1, settings.msToProcess);
      loopCnt_Q_L_P          = zeros(1, settings.msToProcess);
...
// Set up all the code phase tracking information -------------------------
        ...
        tcode2      = ceil(tcode) + 1;
        earlyCode   = E1BCodeSplit(currE1BCodeSegment, tcode2);
        ...
        tcode       = (remCodePhase+earlyLateSpc) : ...
                       ...
                       ((blksize-1)*codePhaseStep+remCodePhase+earlyLateSpc);
        tcode2      = ceil(tcode) + 1;
        lateCode    = E1BCodeSplit(currE1BCodeSegment, tcode2);
        ...
        tcode2      = ceil(tcode) + 1;
        promptCode  = E1BCodeSplit(currE1BCodeSegment, tcode2);
        
        remCodePhase = (tcode(blksize) + codePhaseStep) - 4092.0/4;
        
        currE1BCodeSegment = currE1BCodeSegment+1;
        if (currE1BCodeSegment > 4) then
          currE1BCodeSegment = 1;
        end
...
// Set up all the meandr (subcarrier) phase tracking information ----------
        ...
        remMeandrPhase = (tmeandr(blksize) + meandrPhaseStep) - 8184.0/4;
...
// Find DLL error and update code NCO -------------------------------------
        ...
        //example of PLL/FLL-assisted DLL for GALILEO:
...
// Find SLL error and update code NCO -------------------------------------
        ...
        // Modify code freq based on NCO command
        //example of PLL/FLL-assisted SLL for GALILEO.
