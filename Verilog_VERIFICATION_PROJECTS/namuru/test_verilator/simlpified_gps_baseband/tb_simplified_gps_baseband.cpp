...
#include <stdio.h>
#include <math.h>
#include "Vsimplified_gps_baseband.h"
#include "verilated.h"
...
#define outpw(A,D) wb_write(0x20000000+4*A, D); wait_clock_simple()
#define inpw(A) wb_read(0x20000000+4*A); wait_clock_simple()

#define PRN_KEY			0x00
#define CARR_NCO		0x01
...
#define I_LATE			0x08
#define Q_LATE			0x09
#define CARR_MEAS		0x0A
#define CODE_MEAS		0x0B
#define EPOCH_MEAS		0x0C
#define EPOCH_CHECK		0x0D

#define STATUS			0xE0
#define NEW_DATA		0xE1
#define TIC_COUNT		0xE2
#define ACCUM_COUNT		0xE3

#define RESET			0xF0
#define PROG_TIC		0xF1
...
FILE *extfile;
char EXT_Filename[255] = "/home/Artyom/verilator/FFF005.TXT"; // Name of the file in which data will be recorded.
...
void wait_clock()
{
  ...
  if (start_int_processing) { // start interrupt processing only after correlator is initialized! 
    ...
    if (top->accum_int == 1) {
      ///wb_read(0x20000380); //new_data read.
      accum_task();
      ///start_int_processing = false;
    }
  }

  // read next I/Q samples
  read_sample_from_file(&top->mag, &top->sign); //read I-sample and Q-sample;

  sys_cycle_count++; // go to next time tick;
  
  for (int clk=0; clk<2; clk++) {
    ///tfp->dump ((2*sys_cycle_count+clk)*SYS_CLK_PERIODE);
    top->clk = !top->clk;
    top->eval ();
  }
}
...
void read_sample_from_file(unsigned char *i, unsigned char *q)
{
  ...
  ///*q = ( IF[2*ifdata_curr_pos]   > 0 ? 1 : 0 );
  *q = 0; //[ART] Current signal file is I-samples only!
  *i = ( IF[2*ifdata_curr_pos+1] > 0 ? 1 : 0 );
...
void alloc_task()
{
  ...
  static int sv = 26;//0; //[Art] to reduce acquisition time!
  ...
  ///while (1) {
    ...
    for (i=0; i<MAX_CHANNELS; i++) {
      if (new_data&channel_mask) {
        ...
        fprintf(extfile, "%d, %d, %d, %d, %d, %d, ", IE[i], QE[i], IP[i], QP[i], IL[i], QL[i]);//[Art!]
    ...
    for (i=0; i<MAX_CHANNELS; i++) {
      if (new_data&channel_mask) {
        ...
        if (CH[i].lock_status&CODE_LOCK) {
          ...
          fprintf(extfile, "%d, ", pow_carr);//[ART] for debugging!
        } else {fprintf(extfile, "%d, ", 0);}//[ART] for debugging! }
...
int main(int argc, char **argv, char **env) {
  ...
  top = new Vsimplified_gps_baseband;
  // init trace dump
  ...
  top->trace (tfp, 99);
  tfp->open ("simplified_gps_baseband.vcd");
  ...
  // wishbone bus test:
  wb_write(0x20000300, 0x11111111); wait_clock();
  wb_write(0x20000304, 0x22222222); wait_clock();
  wb_write(0x20000308, 0x33333333); wait_clock();
  wb_write(0x2000030C, 0x44444444); wait_clock();
  wb_write(0x20000310, 0x55555555); wait_clock();
  wb_write(0x20000314, 0x66666666); wait_clock();
  wb_write(0x20000318, 0x77777777); wait_clock();
  wb_write(0x2000031C, 0x88888888); wait_clock();
  wb_read(0x20000340); wait_clock();
  wb_read(0x20000344); wait_clock();
  wb_read(0x20000348); wait_clock();
  wb_read(0x2000034C); wait_clock();
  wb_read(0x20000350); wait_clock();
  wb_read(0x20000354); wait_clock();
  wb_read(0x20000358); wait_clock();
  wb_read(0x2000035C); wait_clock();
...
//=================Takuji Ebinuma code END=======================================
  ...
  wb_write(0x200003C0, 0x00000000); wait_clock();
  ...
  while(run) {
    ...
    if (sys_cycle_count == 8000*80000) run = false;
