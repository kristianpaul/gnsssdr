...
// when tracking has started in a middle of a string.
  ...
  for i = 1:size(trackResults, 2)
    ...
    trkRslt_I_P(i,:)    = trackResults(i).I_P_P((settings.skipNumberOfFirstBits+1):$);
  ...
  [PageStart, activeChnList] =.. 
             findPageStart(trkRslt_status, trkRslt_I_P, set_numberOfChnls);
  ...
  for channelNr = activeChnList
    ...
    if (subFrameStart(channelNr) + (31000) -1) > length(trkRslt_I_P(channelNr,:)) then
      ...
      navBitsSamples = trkRslt_I_P(channelNr, PageStart(channelNr) : ...
                              PageStart(channelNr) + (31000) -1);
      //--- Convert prompt correlator output to +-1 ---------
      navBitsSamples = sign(navBitsSamples);
      //--- Decode data and extract ephemeris information ---
      [eph(trkRslt_PRN(channelNr)), TOW(trkRslt_PRN(channelNr))] = ephemeris(navBitsSamples);
