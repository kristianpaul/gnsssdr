...
gpsisr (void)
{
  ...
  int ch;
	...
	lcd_PrintString("test", "test");
	if ( (cntr02 % 100000) == 0 ){
		if (led02) led02 = led02 & 4294967293; else led02= led02 | 2;
		//LED00 = led02;
	}
...
ch_acq (int ch)
{
  ...
  if (abs (c->n_freq) <= SEARCH_MAX_F) { //search frequencies;
	...
	snprintf(accum_mag_str, 15, "%d", prompt_mag);
	///lcd_PrintString(accum_mag_str, "");
	///printf("MAG = %d", prompt_mag);
    ...
    if (prompt_mag > ACQ_THRESH) {
      ...
      c->codes += 1;
	//lcd_PrintString("next chip", "");

	//printf("n_freq=%d\t codes=%d\n", c->n_freq, c->codes);
    }
	...
	rst_cnt++;
	snprintf(rst_cnt_str, 15, "%d", rst_cnt);
//	if ( (rst_cnt %100) == 0){
		///lcd_PrintString("Reset acq      ", rst_cnt_str);
//	}
    c->n_freq   = 0;
    c->del_freq = 1;
