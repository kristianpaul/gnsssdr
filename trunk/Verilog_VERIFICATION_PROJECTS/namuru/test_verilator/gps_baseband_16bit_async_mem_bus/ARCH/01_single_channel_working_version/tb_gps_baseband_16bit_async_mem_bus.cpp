...
#define STATUS			0xE0
#define NEW_DATA		0xE2
#define TIC_COUNT_LOW		0xE4
#define TIC_COUNT_HIGH		0xE6
#define ACCUM_COUNT_LOW		0xE8
#define ACCUM_COUNT_HIGH	0xEA

#define RESET			0xF0
#define PROG_TIC_LOW		0xF2
#define PROG_TIC_HIGH		0xF4
#define PROG_ACCUM_INT_LOW	0xF6
#define PROG_ACCUM_INT_HIGH	0xF8
...
int main(int argc, char **argv, char **env) {
  ...
  // wishbone bus test:
  wb_write(0x00000100, 0x11111111); wait_clock();
  wb_write(0x00000102, 0x22222222); wait_clock();
  wb_write(0x00000104, 0x33333333); wait_clock();
  wb_write(0x00000106, 0x44444444); wait_clock();
  wb_write(0x00000108, 0x55555555); wait_clock();
  wb_write(0x0000010A, 0x66666666); wait_clock();
  wb_write(0x0000010C, 0x77777777); wait_clock();
  wb_write(0x0000010E, 0x88888888); wait_clock();
  wb_read(0x00000200); wait_clock();
  wb_read(0x00000202); wait_clock();
  wb_read(0x00000204); wait_clock();
  wb_read(0x00000206); wait_clock();
  wb_read(0x00000208); wait_clock();
  wb_read(0x0000020A); wait_clock();
  wb_read(0x0000020C); wait_clock();
  wb_read(0x0000020E); wait_clock();
...
//=================Takuji Ebinuma code END=======================================
  ...
  wb_write(0x000000F0, 0x00000000); wait_clock();//[Art] temporary commented!
